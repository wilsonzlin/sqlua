...
local function OR(...)
    ...
    for _, arg in Utils.iterateArgs(...) do
        ...
        if argType ~= ValueTypes.BOOLEAN then
            error({ message = "Attempted to construct SQL OR expression from non-boolean parts", code = 82 })
        end
        table.insert(sql, "(" .. Tools.getExpressionSQL(arg) .. ")")
    end

    if #sql < 2 then
        error({ message = "Not enough arguments", code = 81 })
    end

    return setmetatable({
        sql = table.concat(sql, " OR "),
